...
class TestRequest(unittest.TestCase):
    ...
    def test_language_of_town_has_to_cut_spending_is_english(self):
        ...
        response = send_request_to_graph_ql(query).get("issue").get("languages").get("uiLocales")
        self.assertEqual(response, "en")

    def test_language_pferdehuhn_is_german(self):
        uid = send_request_to_graph_ql(query_issue_id("pferdehuhn")).get("issue").get("uid")
    ...
    def test_len_of_town_has_to_cut_spending_is_greater_or_equals_29(self):
        ...
        uid = send_request_to_graph_ql(query_uid).get("issue").get("uid")
        query_datas = query_data_of_issue(uid)
        response = send_request_to_graph_ql(query_datas)
